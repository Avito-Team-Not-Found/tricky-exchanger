...
import { fetchExchangeOptions } from '@entities/chain';
...
vi.mock('@entities/chain', async (importOriginal) => {
  ...
  return { ...actual, fetchExchangeOptions: vi.fn() };
});
...
const mockedFetchExchangeOptions = vi.mocked(fetchExchangeOptions);
...
describe('useRequestForm', () => {
  ...
  it('exposes the number of candidate chains found by the backend matching', async () => {
    ...
    // матчинг на бэкенде синхронный: exchange-options уже отдают кандидатные цепочки,
    // их число и попадает в result.matching.createdCandidateChains
    mockedFetchExchangeOptions.mockResolvedValue([
      { chainId: 1, status: 'CANDIDATE' },
      { chainId: 2, status: 'CANDIDATE' },
    ] as never);
    ...
    expect(mockedFetchExchangeOptions).toHaveBeenCalledWith(2);
    expect(result.current.result?.request.id).toBe(2);
    expect(result.current.result?.matching.createdCandidateChains).toBe(2);
  });

  it('falls back to zero candidate chains when the options request fails', async () => {
    mockedCreateRequest.mockResolvedValue({
      id: 2,
      status: 'IN_PROPOSAL',
      offeredItemId: 1,
    ...
    mockedFetchExchangeOptions.mockRejectedValue(new Error('network'));
    const { result } = renderHook(() => useRequestForm(), { wrapper });
