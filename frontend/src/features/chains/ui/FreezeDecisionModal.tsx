...
// Модалка «Готовность к сделке» (SOFT-LOCK §6.1): заголовок, вопрос и кнопки «Да»/«Отказ»/
// «Отмена» блоками в колонку. Открывается через App.useApp().modal — antd-Modal не импортируется
// (no-restricted-imports), а `title` confirm-модалки не используется: он рендерится дважды
// (в заголовке модалки и в confirm-title), поэтому заголовок живёт здесь же в контенте. «Да»
// держит loading, пока не пришёл ответ сервера; модалка закрывается только после него, при ошибке
// остаётся открытой — тост объясняет, что пошло не так. «Отказ» — отказ от сделки, действие
// необратимое, поэтому подтверждается отдельной модалкой (§6.3), а не шлёт запрос сразу.
