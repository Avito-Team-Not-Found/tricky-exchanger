...
func TestFeaturesJsonMatchesGo(t *testing.T) {
	...
	if err := featureNamesEqual(names, FeatureNames); err != nil {
		t.Fatalf("ml/features.json vs FeatureNames: %v", err)
	}
}

func TestModelTextIsV3(t *testing.T) {
	raw, err := os.ReadFile(filepath.Join("models", "ranker_v1.txt"))
	if err != nil {
...
func BenchmarkPredict(b *testing.B) {
	pred, err := NewLightGBMPredictor(filepath.Join("models", "ranker_v1.txt"))
	if err != nil {
		b.Skip(err.Error())
	}
	feats := goldenLikeFeatures()
...
func mustPredictor(t testing.TB) *LightGBMPredictor {
	t.Helper()
	pred, err := NewLightGBMPredictor(filepath.Join("models", "ranker_v1.txt"))
	if err != nil {
		t.Fatalf("NewLightGBMPredictor: %v", err)
	}
	return pred
}
