...
func TestExtractMLFeaturesUsesFormulaSubset(t *testing.T) {
	...
	ml, err := ExtractMLFeatures(s, cfg)
	if err != nil {
		t.Fatal(err)
	}
	if ml["match_mean"] != extracted.Match {
		t.Fatalf("match_mean %v != ExtractFeatures.Match %v", ml["match_mean"], extracted.Match)
	}
...
func TestApplyContextFillsZeroCreatedAt(t *testing.T) {
	...
	s := ApplyContext(ChainState{Count: 2}, ContextSnapshot{
		OfferedCategories: []string{"phones", "laptops"},
		WantedCategories:  []string{"laptops", "phones"},
		CategoryTotal:     10,
	...
	if reasons := SparseChainStateReasons(s); len(reasons) != 0 {
