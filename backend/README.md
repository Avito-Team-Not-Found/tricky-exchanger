...
Миграции запускаются автоматически при старте сервера. Для ручного управления
используется goose (embedded FS):

```bash
# применить вручную (если нужно до запуска сервера)
goose -dir migrations up

# откатить последнюю миграцию (dev)
